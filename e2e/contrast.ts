 ...
 *    boxes are the `.table-shell` wrapper around the validity table and the
 *    `.inspect-block` hex dumps inside the certification modal, both
 *    `overflow-x: auto` around `white-space: pre` content. Content scrolled
 *    past those boxes is not dimmed or partly drawn — it is absent from the frame, and
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks the whole keyring, the validity table, the
     * certification list, the trust graph's node and edge labels and the
     * scenario log, and every one of those nodes re-walks the same handful of
     * ancestors up to `<body>`. Without the caches the pass re-reads the same
     * computed styles and rects thousands of times, and the gate scans
     * seventeen states per configuration. Nothing mutates the DOM during the pass, so the cached
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // different in kind: `#inspect-modal` is a <dialog> that lives in the DOM
      // shut, and the gate opens it with a real click and scans it open AND
      // shut, rather than forcing it as the previous gate forced every
      // <details> and `[hidden]` on the page at once.
     ...
     * hamburger and GitHub glyphs, and the graph's arrowhead marker — would be
